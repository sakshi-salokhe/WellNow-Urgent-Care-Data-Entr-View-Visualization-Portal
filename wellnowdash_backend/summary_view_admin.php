			...
			$x = 0;
				...
				while(($x < $c) and ($y < $num_of_m))
				{
					...
					else
					{
						...
						$dets[$y]['support_sp'] = round($support_sp_count / $c2);
						$dets[$y]['support_enl'] = round($support_enl_count / $c2);
						$dets[$y]['support_dnu'] = round($support_dnu_count / $c2);
						$dets[$y]['support_nyucp'] = round($support_nyucp_count / $c2);
						$dets[$y]['support_nom'] = round($support_nom_count / $c2);
						$dets[$y]['support_emails'] = round($support_emails_count / $c2);
						$dets[$y]['support_deposit_pulls'] = round($support_deposit_pulls_count / $c2);
						$dets[$y]['support_blank_batch_corres'] = round($support_blank_batch_corres_count / $c2);
						$dets[$y]['support_correspondence'] = round($support_correspondence_count / $c2);
						$dets[$y]['support_acct_audits'] = round($support_acct_audits_count / $c2);
						$dets[$y]['support_inv_correct'] = round($support_inv_correct_count / $c2);
						$dets[$y]['support_phone'] = round($support_phone_count / $c2);
						$dets[$y]['support_inv_addr'] = round($support_inv_addr_count / $c2);
						$dets[$y]['support_collects'] = round($support_collects_count / $c2);
						$dets[$y]['suport_medical_records'] = round($suport_medical_records_count / $c2);
						$dets[$y]['coding_na'] = round($coding_na_count / $c2);
						$dets[$y]['coding_on_holds'] = round($coding_on_holds_count / $c2);
						$dets[$y]['coding_coding_queue'] = round($coding_coding_queue_count / $c2);
						$dets[$y]['coding_onsites'] = round($coding_onsites_count / $c2);
						$dets[$y]['coding_ooa'] = round($coding_ooa_count / $c2);
						$dets[$y]['ar120'] = round($ar120_count / $c2);
						$dets[$y]['ar_120percent'] = round($ar_120percent_count / $c2);
						$dets[$y]['ar_90'] = round($ar_90_count / $c2);
						$dets[$y]['ar_voicemails'] = round($ar_voicemails_count / $c2);
						$dets[$y]['ar_unapplied'] = round($ar_unapplied_count / $c2);
						$dets[$y]['ar_audit'] = round($ar_audit_count / $c2);
						$dets[$y]['ar_wbs'] = round($ar_wbs_count / $c2);
