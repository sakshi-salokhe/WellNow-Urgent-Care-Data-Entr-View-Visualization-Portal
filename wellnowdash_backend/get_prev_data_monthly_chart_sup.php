			...
			$cm = 0;
				...
				while(($x < $c) and($y < $num_of_m))
				{
					...
					if($cm == $curr_month)
					{
						...
						$wb_tech_other_goals = is_numeric($emp[$x]['wb_tech_other_goals']) ? $emp[$x]['wb_tech_other_goals'] : 0;
						$wb_demo_elig_goals = is_numeric($emp[$x]['wb_demo_elig_goals']) ? $emp[$x]['wb_demo_elig_goals'] : 0;
						$wb_timely_filing_goals = is_numeric($emp[$x]['wb_timely_filing_goals']) ? $emp[$x]['wb_timely_filing_goals'] : 0;
						$wb_coding_replies_goals = is_numeric($emp[$x]['wb_coding_replies_goals']) ? $emp[$x]['wb_coding_replies_goals'] : 0;
						$wb_sup_reviews_goals = is_numeric($emp[$x]['wb_sup_reviews_goals']) ? $emp[$x]['wb_sup_reviews_goals'] : 0;
						$wb_nf_corres_goals = is_numeric($emp[$x]['wb_nf_corres_goals']) ? $emp[$x]['wb_nf_corres_goals'] : 0;
						$wb_wc_corres_goals = is_numeric($emp[$x]['wb_wc_corres_goals']) ? $emp[$x]['wb_wc_corres_goals'] : 0;
						$waystar_medc_sec_goals = is_numeric($emp[$x]['waystar_medc_sec_goals']) ? $emp[$x]['waystar_medc_sec_goals'] : 0;
						$waystar_oob_goals = is_numeric($emp[$x]['waystar_oob_goals']) ? $emp[$x]['waystar_oob_goals'] : 0;
						$waystar_fidelis_tf_goals = is_numeric($emp[$x]['waystar_fidelis_tf_goals']) ? $emp[$x]['waystar_fidelis_tf_goals'] : 0;
			...
			$cm = 0;
				...
				while(($x < $c) and($y < $num_of_m))
				{
					...
					if($cm == $curr_month)
					{
						...
						$RPQ_print_queue_total_goals = is_numeric($emp[$x]['RPQ_print_queue_total_goals']) ? $emp[$x]['RPQ_print_queue_total_goals'] : 0;
						$RPQ_num_of_WC_goals = is_numeric($emp[$x]['RPQ_num_of_WC_goals']) ? $emp[$x]['RPQ_num_of_WC_goals'] : 0;
						$IL_RCM_Ins_goals = is_numeric($emp[$x]['IL_RCM_Ins_goals']) ? $emp[$x]['IL_RCM_Ins_goals'] : 0;
						$IL_RCM_WC_goals = is_numeric($emp[$x]['IL_RCM_WC_goals']) ? $emp[$x]['IL_RCM_WC_goals'] : 0;
						$IL_NINS_goals = is_numeric($emp[$x]['IL_NINS_goals']) ? $emp[$x]['IL_NINS_goals'] : 0;
						$IL_NNF_goals = is_numeric($emp[$x]['IL_NNF_goals']) ? $emp[$x]['IL_NNF_goals'] : 0;
						$IL_NWC_goals = is_numeric($emp[$x]['IL_NWC_goals']) ? $emp[$x]['IL_NWC_goals'] : 0;
						$IL_ENL_goals = is_numeric($emp[$x]['IL_ENL_goals']) ? $emp[$x]['IL_ENL_goals'] : 0;
						$RA_WS_Prof_goals = is_numeric($emp[$x]['RA_WS_Prof_goals']) ? $emp[$x]['RA_WS_Prof_goals'] : 0;
						$RA_WS_Prof_Day_goals = is_numeric($emp[$x]['RA_WS_Prof_Day_goals']) ? $emp[$x]['RA_WS_Prof_Day_goals'] : 0;
						$RA_WS_Inst_goals = is_numeric($emp[$x]['RA_WS_Inst_goals']) ? $emp[$x]['RA_WS_Inst_goals'] : 0;
						$RA_WS_Inst_Day_goals = is_numeric($emp[$x]['RA_WS_Inst_Day_goals']) ? $emp[$x]['RA_WS_Inst_Day_goals'] : 0;
						$RA_Attachments_goals = is_numeric($emp[$x]['RA_Attachments_goals']) ? $emp[$x]['RA_Attachments_goals'] : 0;
						$WBE_NF_Updates_goals = is_numeric($emp[$x]['WBE_NF_Updates_goals']) ? $emp[$x]['WBE_NF_Updates_goals'] : 0;
						$WBE_WC_Updates_goals = is_numeric($emp[$x]['WBE_WC_Updates_goals']) ? $emp[$x]['WBE_WC_Updates_goals'] : 0;
						$WBE_OS_Email_Inbox_goals = is_numeric($emp[$x]['WBE_OS_Email_Inbox_goals']) ? $emp[$x]['WBE_OS_Email_Inbox_goals'] : 0;
						$Coding_FFS_Total_goals = is_numeric($emp[$x]['Coding_FFS_Total_goals']) ? $emp[$x]['Coding_FFS_Total_goals'] : 0;
						$Coding_FFS_On_hold_goals = is_numeric($emp[$x]['Coding_FFS_On_hold_goals']) ? $emp[$x]['Coding_FFS_On_hold_goals'] : 0;
						$Coding_Coding_Queue_goals = is_numeric($emp[$x]['Coding_Coding_Queue_goals']) ? $emp[$x]['Coding_Coding_Queue_goals'] : 0;
						$Coding_Coding_Queue_Days_goals = is_numeric($emp[$x]['Coding_Coding_Queue_Days_goals']) ? $emp[$x]['Coding_Coding_Queue_Days_goals'] : 0;
						$Coding_WS_Coding_goals = is_numeric($emp[$x]['Coding_WS_Coding_goals']) ? $emp[$x]['Coding_WS_Coding_goals'] : 0;
						$Coding_FFS_Onhold_Report_goals = is_numeric($emp[$x]['Coding_FFS_Onhold_Report_goals']) ? $emp[$x]['Coding_FFS_Onhold_Report_goals'] : 0;
