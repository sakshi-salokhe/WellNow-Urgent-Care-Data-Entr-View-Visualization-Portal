...
{
	$sql2 = "select * from ar_goals where user_id = '$userid' and when_done = '$today' and when_done_year = '$today_year'";
	...
	if($count > 0)
	{
		$emp['wb_tech_other_PH'] = $row2['wb_tech_other'];
		$emp['wb_demo_elig_PH'] = $row2['wb_demo_elig'];
		$emp['wb_timely_filing_PH'] = $row2['wb_timely_filing'];
		$emp['wb_coding_replies_PH'] = $row2['wb_coding_replies'];
		$emp['wb_sup_reviews_PH'] = $row2['wb_sup_reviews'];
		$emp['wb_nf_corres_PH'] = $row2['wb_nf_corres'];
		$emp['wb_wc_corres_PH'] = $row2['wb_wc_corres'];
		$emp['waystar_medc_sec_PH'] = $row2['waystar_medc_sec'];
		$emp['waystar_oob_PH'] = $row2['waystar_oob'];
		$emp['waystar_fidelis_tf_PH'] = $row2['waystar_fidelis_tf'];
	}
	else{
...
{
	$sql2 = "select * from os_goals where user_id = '$userid' and when_done = '$today' and when_done_year = '$today_year'";
	$result2 = mysqli_query($con,$sql2);
	$row2 = mysqli_fetch_array($result2);
	...
	if($count > 0)
	{
		$emp['RPQ_print_queue_total_PH'] = $row2['RPQ_print_queue_total'];
		$emp['RPQ_num_of_WC_PH'] = $row2['RPQ_num_of_WC'];
		$emp['IL_RCM_Ins_PH'] = $row2['IL_RCM_Ins'];
		$emp['IL_RCM_WC_PH'] = $row2['IL_RCM_WC'];
		$emp['IL_NINS_PH'] = $row2['IL_NINS'];
		$emp['IL_NNF_PH'] = $row2['IL_NNF'];
		$emp['IL_NWC_PH'] = $row2['IL_NWC'];
		$emp['IL_ENL_PH'] = $row2['IL_ENL'];
		$emp['RA_WS_Prof_PH'] = $row2['RA_WS_Prof'];
		$emp['RA_WS_Prof_Day_PH'] = $row2['RA_WS_Prof_Day'];
		$emp['RA_WS_Inst_PH'] = $row2['RA_WS_Inst'];
		$emp['RA_WS_Inst_Day_PH'] = $row2['RA_WS_Inst_Day'];
		$emp['RA_Attachments_PH'] = $row2['RA_Attachments'];
		$emp['WBE_NF_Updates_PH'] = $row2['WBE_NF_Updates'];
		$emp['WBE_WC_Updates_PH'] = $row2['WBE_WC_Updates'];
		$emp['WBE_OS_Email_Inbox_PH'] = $row2['WBE_OS_Email_Inbox'];
		$emp['Coding_FFS_Total_PH'] = $row2['Coding_FFS_Total'];
		$emp['Coding_FFS_On_hold_PH'] = $row2['Coding_FFS_On_hold'];
		$emp['Coding_Coding_Queue_PH'] = $row2['Coding_Coding_Queue'];
		$emp['Coding_Coding_Queue_Days_PH'] = $row2['Coding_Coding_Queue_Days'];
		$emp['Coding_WS_Coding_PH'] = $row2['Coding_WS_Coding'];
		$emp['Coding_FFS_Onhold_Report_PH'] = $row2['Coding_FFS_Onhold_Report'];
	}
	else{
...
{
	$sql2 = "select * from om_goals where user_id = '$userid' and when_done = '$today' and when_done_year = '$today_year'";
	$result2 = mysqli_query($con,$sql2);
	$row2 = mysqli_fetch_array($result2);
	...
	if($count > 0)
	{
		$emp['support_sp_PH'] = $row2['support_sp'];
		$emp['support_enl_PH'] = $row2['support_enl'];
		$emp['support_dnu_PH'] = $row2['support_dnu'];
		$emp['support_nyucp_PH'] = $row2['support_nyucp'];
		$emp['support_nom_PH'] = $row2['support_nom'];
		$emp['support_emails_PH'] = $row2['support_emails'];
		$emp['support_deposit_pulls_PH'] = $row2['support_deposit_pulls'];
		$emp['support_blank_batch_corres_PH'] = $row2['support_blank_batch_corres'];
		$emp['support_correspondence_PH'] = $row2['support_correspondence'];
		$emp['support_acct_audits_PH'] = $row2['support_acct_audits'];
		$emp['support_inv_correct_PH'] = $row2['support_inv_correct'];
		$emp['support_phone_PH'] = $row2['support_phone'];
		$emp['support_inv_addr_PH'] = $row2['support_inv_addr'];
		$emp['support_collects_PH'] = $row2['support_collects'];
		$emp['suport_medical_records_PH'] = $row2['suport_medical_records'];
		
		$emp['coding_na_PH'] = $row2['coding_na'];
		$emp['coding_on_holds_PH'] = $row2['coding_on_holds'];
		$emp['coding_coding_queue_PH'] = $row2['coding_coding_queue'];
		$emp['coding_onsites_PH'] = $row2['coding_onsites'];
		$emp['coding_ooa_PH'] = $row2['coding_ooa'];
		
		$emp['ar120_PH'] = $row2['ar120'];
		$emp['ar_120percent_PH'] = $row2['ar_120percent'];
		$emp['ar_90_PH'] = $row2['ar_90'];
		$emp['ar_voicemails_PH'] = $row2['ar_voicemails'];
		$emp['ar_unapplied_PH'] = $row2['ar_unapplied'];
		$emp['ar_audit_PH'] = $row2['ar_audit'];
		$emp['ar_wbs_PH'] = $row2['ar_wbs'];
	}
	else{
...
{
	$sql2 = "select * from pat_sup_goals where user_id = '$userid' and when_done = '$today' and when_done_year = '$today_year'";
	$result2 = mysqli_query($con,$sql2);
	$row2 = mysqli_fetch_array($result2);
	...
	if($count > 0)
	{
		$emp['saf_mvp_sod_PH'] = $row2['saf_mvp_sod'];
		$emp['saf_inval_addr_sod_PH'] = $row2['saf_inval_addr_sod'];
		$emp['ash_attachments_sod_PH'] = $row2['ash_attachments_sod'];
		$emp['ash_wc_mailing_sod_PH'] = $row2['ash_wc_mailing_sod'];
		$emp['ash_wc_deleted_sod_PH'] = $row2['ash_wc_deleted_sod'];
		$emp['ash_acc_type_sod_PH'] = $row2['ash_acc_type_sod'];
		$emp['ash_last_addr_sod_PH'] = $row2['ash_last_addr_sod'];
		$emp['bailey_indep_health_sod_PH'] = $row2['bailey_indep_health_sod'];
		$emp['bailey_bcbs_sod_PH'] = $row2['bailey_bcbs_sod'];
		$emp['bailey_emails_sod_PH'] = $row2['bailey_emails_sod'];
		$emp['justin_ndc_num_sod_PH'] = $row2['justin_ndc_num_sod'];
		$emp['justin_medicare_loc_sod_PH'] = $row2['justin_medicare_loc_sod'];
		$emp['justin_medicare_sec_sod_PH'] = $row2['justin_medicare_sec_sod'];
	}
	else{
