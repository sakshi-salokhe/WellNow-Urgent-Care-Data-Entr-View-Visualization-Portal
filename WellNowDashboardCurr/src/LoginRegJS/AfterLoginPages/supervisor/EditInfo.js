...
class EditInfo extends Component
{
	...
	handleSubmit(event)
	{
		...
		if(obj.fullname.length === 0 || obj.dashboards.length === 0 || obj.email.length === 0)
		{
			alert("Fill out all the fields!")
		}
		else
		{
			axios.post('http://localhost:81/wellnowdash_backend/update_after_editing_sup.php', qs.stringify(obj))
			.then(res => 
			{
				...
				if(res.data.done === 1)
				{
					...
					ReactDOM.render(<ApproveNewUsersSuper user_id = {this.props.userid.supervisor_id}/>, document.getElementById('root'));
				}
				else
				{
